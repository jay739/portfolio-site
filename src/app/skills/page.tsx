...
const SkillsChart = dynamic(() => import("@/components/sections/SkillsChart"), {
  ssr: false,
  loading: () => (
    <div className="w-full min-h-[520px] neural-card-soft rounded-xl animate-pulse" />
  ),
});

const NeuralNetworkViz = dynamic(
  () => import("@/components/ui/NeuralNetworkViz"),
  {
    ssr: false,
    loading: () => (
      <div className="w-full min-h-[560px] neural-card-soft rounded-xl animate-pulse" />
    ),
  },
);
    ...
    <main className="flex min-h-screen flex-col items-center justify-between neural-page-shell gap-4 sm:gap-5">
      <RecentViewTracker
        item={{
          ...
          href: "/skills",
          kind: "page",
          description: "Interactive skill topology and domain map",
        }}
      />
      <NeuralPageIntro
        title="Skills Graph"
      ...
      <div className="w-full rounded-[28px]">
        <section
          id="skills-map"
          className="w-full neural-card neural-glow-border p-4 sm:p-6 md:p-8"
        >
          <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
            <h2 className="neural-section-title text-xl sm:text-2xl">
          ...
          <div className="mb-5 rounded-xl border border-amber-400/20 bg-slate-900/50 p-4">
            <p className="text-[10px] font-bold uppercase tracking-widest text-amber-300 mb-3">
              How this map works
            </p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
              {[
                {
                  label: "INPUT",
                  sub: "Core skills & tools",
                  color: "#D97706",
                  note: "11 nodes",
                },
                {
                  label: "LAYER 1",
                  sub: "Derived capabilities",
                  color: "#F97316",
                  note: "9 nodes",
                },
                {
            ...
            <p className="text-[11px] text-slate-400 leading-relaxed">
              Edges flow{" "}
              <span className="text-amber-300 font-medium">left → right</span>:
              raw skills combine into capabilities, which power projects, which
              deliver outcomes. Signal dots animate along each edge to show
              active data flow.{" "}
              <span className="text-slate-300">Hover a node</span> to read its
              role · <span className="text-slate-300">Click</span> to pin &
              isolate · <span className="text-slate-300">Double-click</span> to
              navigate · use{" "}
              <span className="text-slate-300">Filter Domains</span> below to
      ...
      <div className="w-full rounded-[28px]">
        <section id="skills" className="relative w-full p-0">
      ...
      <RouteNextSteps
        items={[
          {
            href: "/projects",
            label: "See matching projects",
            note: "Move from domain strengths into real case studies.",
          },
          {
            href: "/impact",
            label: "Tie skills to outcomes",
            note: "See where these capabilities produced measurable results.",
          },
          {
            href: "/paths",
            label: "Follow a curated collection",
            note: "Choose a guided route based on your interests.",
          },
        ]}
      />
    </main>

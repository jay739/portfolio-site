...
const HomeServerStats = dynamic(
  () => import("@/components/sections/HomeServerStats"),
  {
    ssr: false,
    loading: () => (
...
const HomelabArchitecturePanel = dynamic(
  () => import("@/components/sections/HomelabArchitecturePanel"),
  {
    ssr: false,
    loading: () => (
      <div className="w-full min-h-[520px] neural-card-soft rounded-xl animate-pulse" />
    ...
    <main className="flex min-h-screen flex-col items-center justify-between neural-page-shell gap-4 sm:gap-5">
      <RecentViewTracker
        item={{
          ...
          href: "/homeserver",
          kind: "page",
          description: "Live infrastructure metrics and homelab telemetry",
        }}
      />
      <NeuralPageIntro
        title="Telemetry Control Center"
      ...
      <section id="home-server" className="relative w-full">
        <div className="absolute right-3 top-3 z-20">
        ...
        items={[
          {
            ...
            label: "Open the Batcave case study",
            note: "See the architecture story behind the dashboard.",
          },
          {
            href: "/skills?mode=map&domain=DevOps",
            label: "Map the infra skills",
            note: "Follow telemetry back to the DevOps/MLOps stack.",
          },
          {
            href: "/ai-tools",
            label: "Use the AI tools",
            note: "Move from ops visibility into the live product surfaces.",
          },
        ]}
      />
    </main>

...
const tools: Tool[] = [
  {
    ...
    description:
      "Upload a PDF, get back a multi-host podcast with transcript. Ollama + MARS5-TTS on Apple M4.",
  ...
  },
  {
    title: "RAG Chatbot",
    description:
      "Chatbot with retrieval-augmented generation, trained on my docs.",
    action: "open-chatbot",
    enabled: true,
  },
  {
    ...
    description:
      "Generate images from text prompts using Stable Diffusion running on Apple M4.",
...
const COMPUTE_LABEL: Record<"rtx" | "apple-silicon", string> = {
  rtx: "RTX GPU",
...
export default function AiToolsLab() {
  ...
  const [imgSpeedMode, setImgSpeedMode] = useState<
  ...
  const [imgCompute, setImgCompute] = useState<"rtx" | "apple-silicon" | null>(
    null,
  );
  ...
  const speedModes: {
  ...
  }[] = [
    {
      ...
      label: (
        <>
          <FaBolt className="inline text-amber-400 mr-1" />
          Fast
        </>
      ),
      desc: "SD Turbo · ~45–60s · 512px",
    },
    {
      key: "balanced",
      label: (
        <>
          <FaBalanceScale className="inline text-amber-400 mr-1" />
          Balanced
        </>
      ),
      desc: "SD 1.5 · ~60–100s · 640px",
    },
    {
      key: "quality",
      label: (
        <>
          <FaGem className="inline text-amber-400 mr-1" />
          Quality
        </>
      ),
      desc: "SDXL Turbo · ~2–4 min · 1024px",
    },
  ];
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (
        queued.meta?.compute === "rtx" ||
        queued.meta?.compute === "apple-silicon"
      ) {
        setImgCompute(queued.meta.compute);
      }
      ...
      while (Date.now() < deadline) {
        ...
        try {
          sRes = await fetch(
            `/api/generate-image?id=${encodeURIComponent(promptId)}`,
          );
        } catch {
          continue;
        }
        const s = await sRes
          .json()
          .catch(() => ({ status: "error", error: "Non-JSON response" }));
        if (s.status === "done") {
          ...
          if (s.compute === "rtx" || s.compute === "apple-silicon") {
            setImgCompute(s.compute);
          }
          setImgProgress(100);
          return;
        }
        if (s.status === "error" && sRes.status === 404) {
          ...
          throw new Error(
            "Generation failed — job not found on GPU server. Try again.",
          );
        }
        if (s.status === "error") {
      ...
      throw new Error(
        "Generation timed out (6 min). The GPU may be busy — try Fast mode.",
      );
    } catch (err: unknown) {
      ...
      if (msg.includes("Failed to fetch") || msg.includes("NetworkError")) {
        ...
        setImgError(
          "Request timed out — the GPU may be overloaded. Try again in a moment.",
        );
      } else {
        setImgError(msg);
      }
  ...
  const saveCurrentPreset = () => {
    ...
    const deduped = savedPresets.filter(
  ...
  const copyPresetLink = async () => {
    ...
    setPresetNotice(
      ok ? "Preset link copied to clipboard" : "Unable to copy preset link",
    );
  };
  ...
  const renderSection = (
    <section className="relative pt-0 pb-16 px-2 sm:px-6 w-full overflow-hidden">
      ...
      <div className="w-full neural-card neural-glow-border p-4 sm:p-8">
        ...
        />
        <h2 className="neural-section-title mb-3 flex items-center gap-2">
          <FaFlask className="text-amber-400" /> AI Tools Lab
        </h2>
        <p className="neural-section-copy max-w-4xl mb-3">
          Experimental AI-powered tools showcasing LLMs, RAG systems, and
          generative image models — all running on self-hosted infrastructure
          powered by an Apple M4 Mac Mini with Metal GPU acceleration.
        </p>
        <div className="mb-6 flex flex-wrap items-center gap-2">
          ...
          {[
            ...
            <button
              ...
              type="button"
              onClick={() =>
                setToolFilter(filter.key as "all" | "live" | "soon")
              }
        ...
        <div className="mb-8 text-sm text-slate-300">
          Showing {filteredTools.length} tools. Live:{" "}
          {tools.filter((t) => t.enabled).length} | In progress:{" "}
      ...
      <AnimatePresence>
        ...
        {imgModalOpen && (
          <motion.div
            ...
            onClick={(e) => {
              if (e.target === e.currentTarget) setImgModalOpen(false);
            }}
          >
            <motion.div
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700/60 shrink-0">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    SD · FLUX.1 Schnell · RTX GPU + Apple Silicon · Self-hosted
                    (ComfyUI)
                  </p>
                </div>
                <button
              ...
              <div className="flex flex-col lg:flex-row flex-1 min-h-0 overflow-hidden">
                ...
                <div className="lg:w-[420px] shrink-0 overflow-y-auto p-5 border-b lg:border-b-0 lg:border-r border-slate-700/50 space-y-4">
                        ...
                        [
                          {
                            ...
                            desc: "SD 1.5 · Turbo · SDXL",
                          },
                          {
                            key: "flux",
                            label: "FLUX.1 Schnell",
                            desc: "4-step · Higher quality",
                          },
                        ] as {
                          key: "sd" | "flux";
                          label: string;
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="grid grid-cols-1 gap-2">
                        {(["square", "portrait", "landscape"] as const).map(
                          (a) => (
                            <button
                              key={a}
                              type="button"
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {(
                        [
                          ...
                          {
                            key: "photorealistic",
                            label: <FaCamera className="text-amber-400" />,
                            desc: "Photoreal",
                          },
                          {
                            key: "cinematic",
                            label: <FaFilm className="text-amber-400" />,
                            desc: "Cinematic",
                          },
                          {
                            key: "anime",
                            label: <FaStar className="text-amber-400" />,
                            desc: "Anime",
                          },
                          {
                            key: "cyberpunk",
                            label: <FaBuilding className="text-amber-400" />,
                            desc: "Cyberpunk",
                          },
                          {
                            key: "pixel-art",
                            label: <FaGamepad className="text-amber-400" />,
                            desc: "Pixel art",
                          },
                        ] as {
                          key:
                      ...
                      <div className="space-y-3 p-3 rounded-lg border border-slate-700 bg-slate-900/40">
                        ...
                        <div>
                          ...
                          <input
                            value={imgSeed}
                            onChange={(e) =>
                              setImgSeed(e.target.value.replace(/[^0-9]/g, ""))
                            }
                            placeholder="e.g. 42"
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        ...
                        <p className="text-[11px] text-slate-500">
                          Keep favorite generator setups and share them as
                          links.
                        </p>
                      </div>
                      <button
                    ...
                    {savedPresets.length > 0 ? (
                      <div className="space-y-2">
                        {savedPresets.map((preset) => (
                          <div
                            key={preset.id}
                            className="rounded-lg border border-slate-700/70 bg-slate-950/40 p-2.5"
                          >
                            <div className="flex items-start justify-between gap-2">
                              <div>
                                ...
                                <p className="mt-1 text-[10px] text-slate-500">
                                  {preset.speedMode} · {preset.quality} ·{" "}
                                  {preset.aspect} · {preset.style}
                                </p>
                              </div>
                              <button
                                type="button"
                      ...
                      <p className="text-[11px] text-slate-500">
                        No presets saved yet. Save one to reuse your favorite
                        generation setup.
                      </p>
                    )}
                  </div>
                  ...
                  <button
                    ...
                    disabled={
                      imgLoading || imgOtherTabBusy || !imgPrompt.trim()
                    }
                    className="neural-pill w-full justify-center disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {imgLoading ? (
                      <>
                        <FaSpinner className="inline animate-spin text-amber-400 mr-1.5" />
                        Generating
                        {imgCompute
                          ? ` on ${COMPUTE_LABEL[imgCompute]}`
                          : "..."}
                      </>
                    ) : (
                      <>
                        <FaMagic className="inline text-amber-400 mr-1.5" />
                        Generate Image
                      </>
                    )}
                  </button>
                  ...
                  {imgLoading && (
                    <div>
                      ...
                      <div className="text-center text-xs text-slate-400 mt-1.5 animate-pulse">
                        {imgProgress < 92
                          ? `Running on ${imgCompute ? COMPUTE_LABEL[imgCompute] : "GPU"}… ${Math.round(imgProgress)}%`
                          : "Finalizing…"}
                      </div>
                    </div>
                  )}
                  ...
                  {imgError && (
                    <div className="p-4 rounded-xl border border-red-500/30 bg-red-500/10">
                      ...
                      <div className="text-[11px] text-slate-400 text-center mb-3">
                        {imgError.includes("timed out")
                          ? 'The GPU took too long. Try "Fast" mode or a simpler prompt.'
                          : imgError.includes("unreachable") ||
                              imgError.includes("fetch")
                            ? "ComfyUI server might be sleeping. Give it a moment and try again."
                            : imgError.includes("Rate limit")
                              ? "Too many requests. Wait a minute before trying again."
                              : imgError.includes("content policy")
                                ? "This prompt is not allowed. Please try something different."
                                : "Something went wrong. Try again or use a different prompt."}
                      </div>
                      <button
                        type="button"
                        onClick={() => setImgError(null)}
                        className="neural-pill text-xs mx-auto flex items-center gap-1"
                      >
                        <FaTimes className="inline text-amber-400 mr-1" />{" "}
                        Dismiss
                      </button>
                    </div>
                  )}
                  ...
                  {!imgResult && !imgLoading && (
                    <div className="flex flex-col items-center justify-center gap-3 text-center opacity-40 select-none">
                      ...
                      <p className="text-xs text-slate-500">
                        Configure settings on the left and hit Generate
                      </p>
                    </div>
                  )}
                  ...
                  {imgLoading && !imgResult && (
                    <div className="flex flex-col items-center justify-center gap-4">
                      ...
                      <p className="text-sm text-slate-400 animate-pulse">
                        Rendering
                        {imgCompute ? ` on ${COMPUTE_LABEL[imgCompute]}` : "…"}
                      </p>
                    </div>
                  )}
                    ...
                    <div className="w-full flex flex-col items-center gap-4">
                      ...
                      >
                        ...
                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition flex items-center justify-center opacity-0 group-hover:opacity-100">
                          <span className="text-white text-sm flex items-center gap-1.5 bg-black/60 px-3 py-1.5 rounded-full">
                            <FaSearch className="text-amber-400" /> Click to
                            enlarge
                          </span>
                        </div>
                      </button>
                      ...
                      {imgMeta && (
                        <div className="text-[10px] text-slate-500 text-center">
                          {imgMeta.width}×{imgMeta.height} · {imgMeta.steps}{" "}
                          steps · seed {imgMeta.seed} ·{" "}
                          {(imgMeta.model ?? "").replace(".safetensors", "")}
                          {imgCompute && (
                            <>
                              {" "}
                              ·{" "}
                              <span className="text-amber-400/80">
                                {COMPUTE_LABEL[imgCompute]}
                              </span>
                            </>
                          )}
                        </div>
                      ...
                      <div className="flex flex-wrap gap-2 justify-center">
                        ...
                        >
                          <FaSearch className="inline text-amber-400 mr-1" />{" "}
                          View
                        </button>
                        <a
                          href={imgResult}
                        ...
                        <a
                          ...
                          className="neural-pill text-sm"
                        >
                          <FaDownload className="inline text-amber-400 mr-1" />{" "}
                          Download
                        </a>
                        <button
                          type="button"
                          ...
                          disabled={
                            imgSaved || imgSaving || !imgResult || !imgMeta
                          }
                          className={`neural-pill text-sm ${imgSaved || !imgMeta ? "opacity-70 cursor-not-allowed" : ""}`}
                        >
                          {imgSaved ? (
                            <>
                              <FaCheckCircle className="inline text-amber-400 mr-1" />
                              Saved!
                            </>
                          ) : imgSaving ? (
                            <>
                              <FaSave className="inline text-amber-400 mr-1" />
                              Saving…
                            </>
                          ) : (
                            <>
                              <FaImages className="inline text-amber-400 mr-1" />
                              Save to Showcase
                            </>
                          )}
                        </button>
            ...
            >
              ...
              <div className="text-center mb-4">
                ...
                <p className="text-xs text-slate-400 mt-1">
                  Enter the gallery password to save this image.
                </p>
              </div>
              <input
                type="password"

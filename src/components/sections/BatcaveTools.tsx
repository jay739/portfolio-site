  ...
  return (
    ...
    >
      {copied ? (
        ...
        <FaCopy className="text-amber-400" />
      )}
      {copied ? "Copied!" : "Copy"}
    </button>
  );
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-slate-400">
        Generate RFC 4122 v4 UUIDs using the browser&apos;s cryptographic PRNG.
      </p>
      <div className="flex items-center gap-2 flex-wrap">
        <code
          suppressHydrationWarning
          className="flex-1 min-w-0 truncate text-amber-300 text-sm font-mono bg-slate-800/60 border border-slate-700/60 rounded-lg px-3 py-2"
        >
          {uuid}
        </code>
        <CopyBtn value={uuid} />
        <button
          ...
          className="neural-pill py-1 px-3 text-xs flex items-center gap-1.5"
        >
      ...
      {count > 1 && (
        <div className="relative">
          <pre
            suppressHydrationWarning
            className="text-xs text-slate-300 font-mono bg-slate-800/60 border border-slate-700/60 rounded-lg p-3 whitespace-pre-wrap"
          >
            {bulk}
          </pre>
...
function HashTool() {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-slate-400">
        SHA-256 hash computation — same cryptographic functions powering
        Batcave&apos;s internal services.
      </p>
      <textarea
        value={input}
        onChange={(e) => {
      ...
      <button
        ...
        className="neural-pill text-xs flex items-center gap-1.5"
      >
        <FaHashtag className="text-amber-400" />{" "}
        {loading ? "Hashing..." : "Compute SHA-256"}
      </button>
      {hash && (
        <div className="flex items-center gap-2">
          <code className="flex-1 min-w-0 truncate text-amber-300 text-xs font-mono bg-slate-800/60 border border-slate-700/60 rounded-lg px-3 py-2">
            {hash}
          </code>
          <CopyBtn value={hash} />
        </div>
...
function Base64Tool() {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-slate-400">
        Encode or decode Base64 — supports Unicode (UTF-8 safe).
      </p>
      ...
      {input && !error && (
        <div className="relative">
          <pre className="text-xs text-slate-300 font-mono bg-slate-800/60 border border-slate-700/60 rounded-lg p-3 whitespace-pre-wrap break-all">
            {output}
          </pre>
          <div className="absolute top-2 right-2">
...
function PasswordTool() {
  ...
  const strength = (() => {
    if (pwd.length >= 20 && Object.values(opts).every(Boolean))
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-slate-400">
        Cryptographically random password using{" "}
      ...
      <div className="flex items-center gap-3">
        <label className="text-xs text-slate-400 shrink-0">
          Length: <span className="text-amber-400">{len}</span>
        </label>
        <input
          type="range"
          min={8}
          ...
          onChange={(e) => {
            ...
            setPwd(generatePassword(+e.target.value, opts));
          }}
          className="flex-1 accent-amber-400"
        />
      </div>
      <div className="flex flex-wrap gap-2">
        ...
        <span
          className={`ml-auto text-xs font-semibold self-center ${strength.color}`}
        >
          {strength.label}
        </span>
      </div>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-3">
        ...
        />
        <div
          className="w-10 h-10 rounded-lg border border-slate-700/60 shrink-0"
      ...
      {rgb && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
          {[
            ...
            <div
              key={label}
              className="flex items-center justify-between gap-2 bg-slate-800/60 border border-slate-700/60 rounded-lg px-3 py-2"
            >
              <span className="text-xs text-slate-400 shrink-0 w-8">
                {label}
              </span>
      ...
      <div className="flex flex-wrap gap-2">
        {modes.map((m) => {
          ...
          return (
            <div
              key={m.key}
              className="flex items-center gap-1 bg-slate-800/60 border border-slate-700/60 rounded-lg pl-3 pr-1.5 py-1"
            >
              <span className="text-xs text-slate-400 shrink-0 w-20 font-mono">
                {m.label}
              </span>
              <code className="text-xs text-amber-300 font-mono truncate max-w-[160px]">
                {result || "…"}
              </code>
...
function TempTool() {
  ...
  const results = units
    ...
    .map((u) => ({
      unit: u,
      value: valid ? convertTemp(num, from, u).toFixed(2) : "—",
    }));
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-3">
        {results.map(({ unit, value: v }) => (
          <div
            key={unit}
            className="flex items-center gap-2 bg-slate-800/60 border border-slate-700/60 rounded-lg px-3 py-2"
          >
            <span className="text-xs text-slate-400">°{unit}</span>
            <code className="text-amber-300 text-sm font-mono font-bold">
              {v}
            </code>
...
function WordCountTool() {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-slate-400">
        Real-time text statistics and readability estimate.
      </p>
      <textarea
        value={input}
        onChange={(e) => setInput(e.target.value)}
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
        {[
          ...
          <div
            key={label}
            className="flex flex-col items-center bg-slate-800/60 border border-slate-700/60 rounded-lg py-2 px-1"
          >
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="text-xs text-slate-500">
        Showing {tools.length} of {suiteName === "IT Tools" ? "60+" : "30+"}{" "}
        tools available on Batcave
      </div>
    </div>
  );
}
...
const SUITES = [
  {
    ...
    description:
      "Developer toolkit running on Batcave — UUID generator, Base64, hash functions, password generator, JWT decoder, regex tester, and more. Powered by the same infrastructure behind my self-hosted services.",
  ...
  },
  {
    ...
    description:
      "Multi-format conversion suite on Batcave — color spaces, text case transformers, temperature converter, word counter, and more. Part of my self-hosted utility stack.",
    ...
    <section className="relative pt-4 pb-16 px-2 sm:px-6 w-full">
      <div className="w-full neural-card neural-glow-border p-4 sm:p-8">
          ...
          <div>
            ...
            <p className="neural-section-copy max-w-2xl mt-1">
              ...
              <span className="text-amber-400 font-semibold">
                56-container Batcave
              </span>{" "}
              server — a private self-hosted infrastructure stack running 24/7.
              Processing happens on Batcave.
            </p>
          </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-4 pb-4 border-b border-slate-700/40">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-400 max-w-lg mt-0.5">
                    {suite.description}
                  </p>
                </div>
              </div>
              <div className="flex flex-wrap gap-1.5 shrink-0">
                {suite.stack.map((s) => (
                  <span
                    key={s}
                    className="neural-pill neural-pill-static text-xs py-0.5 px-2"
                  >
                    {s}
                  </span>
                ))}
        ...
        <div className="mt-6 flex items-start gap-2 p-3 rounded-xl bg-slate-800/40 border border-slate-700/40 text-xs text-slate-400">
          ...
          <span>
            These tools are part of a larger self-hosted utility stack running
            on Batcave — a private infrastructure accessible over a secure VPN.
            The demos above showcase a selection of what&apos;s available on the
            server.
          </span>
        </div>
      </div>

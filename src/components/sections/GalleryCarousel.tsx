...
export default function GalleryCarousel({
  ...
  // Collapse the prompt back to 2 lines whenever the lightbox image changes.
  useEffect(() => {
  ...
  const models = useMemo(
    () => [
      ...
      ...Array.from(
        new Set(
          items.map((item) => (item.model ?? "").replace(".safetensors", "")),
        ),
      ).sort(),
    ],
    [items],
  ...
  const filteredItems = useMemo(
    () =>
      items.filter((item) => {
        ...
        const matchesModel =
          selectedModel === "all" || cleanModel === selectedModel;
        const matchesOrientation =
          selectedOrientation === "all" ||
          getOrientation(item) === selectedOrientation;
  ...
  const lightbox =
    activeItems.find((item) => item.id === lightboxId) ??
    items.find((item) => item.id === lightboxId) ??
    null;
  const lightboxIndex = lightbox
    ? activeItems.findIndex((item) => item.id === lightbox.id)
    : -1;
  ...
  const toggleCompare = (itemId: string) => {
    setCompareIds((prev) => {
      ...
      pushSiteFeedback(
        prev.length === 1
          ? "Compare tray ready with two images."
          : "Added image to compare tray.",
        "success",
      );
        ...
        <div className="w-full neural-card neural-glow-border p-6 sm:p-8">
          ...
          <div className="flex gap-5">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="w-72 h-72 shrink-0 rounded-xl bg-slate-700/30 animate-pulse"
              />
            ))}
          </div>
        </div>
      ...
      <section className="relative py-12 px-2 sm:px-6 w-full">
        <div className="w-full neural-card neural-glow-border p-6 sm:p-8">
          ...
          <p className="neural-section-copy max-w-3xl">
            Gallery items are not loading right now. If images exist on disk,
            this usually means the runtime fetch failed or returned an empty
            manifest.
          </p>
          <button
            type="button"
            onClick={() => void fetchGallery()}
            className="mt-4 neural-control-btn text-sm"
          >
            Retry gallery load
          </button>
        </div>
    ...
    <section className="relative py-12 px-2 sm:px-6 w-full">
      <div className="w-full neural-card neural-glow-border p-4 sm:p-8">
        ...
        <div className="flex items-center justify-between mb-2">
          <h2 className="neural-section-title mb-0 flex items-center gap-2">
            <FaImages className="text-amber-400" /> AI Gallery
          </h2>
          <div className="flex items-center gap-2">
            ...
            >
              {paused ? (
                <>
                  <FaPlay className="inline text-amber-400 mr-1" />
                  Play
                </>
              ) : (
                <>
                  <FaPause className="inline text-amber-400 mr-1" />
                  Pause
                </>
              )}
            </button>
        ...
        <p className="neural-section-copy max-w-3xl mb-6">
          Generated with Stable Diffusion on self-hosted Apple M4 Metal GPU —
          each image crafted via the AI Image Generator above.
        </p>
        ...
        <div className="mb-6 grid gap-4 rounded-2xl border border-slate-700/60 bg-slate-950/30 p-4">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {(
                  ["all", "square", "portrait", "landscape"] as Orientation[]
                ).map((orientation) => (
                  <FilterChip
                    key={orientation}
        ...
        {compareItems.length > 0 && (
          <div
            className={`mb-6 grid gap-4 ${compareItems.length > 1 ? "lg:grid-cols-2" : "grid-cols-1"}`}
          >
            {compareItems.map((item) => (
              <div
                key={item.id}
                className="rounded-2xl border border-slate-700/60 bg-slate-950/35 p-4"
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs uppercase tracking-widest text-amber-300">
                      {item.style === "none" ? "No style" : item.style}
                    </p>
                    <p className="mt-1 text-xs text-slate-400">
                      {(item.model ?? "").replace(".safetensors", "")} ·{" "}
